...
class nestselectutil():
                ...
                for key in dic.keys():
                    if key == objkey:
                        ...
                        if isinstance(dic[key], dict):
                            new_key = nestselectutil().get_dic_nest(dic[key], objkey=objkey, default=default, values=values)
                ...
                for k, v in dic.items():
                    if v == values:
                        ...
                        if isinstance(v, dict):
                            new_v = nestselectutil().get_dic_nest(v,values=values)
    ...
    def diguichaxun(self, lis_data=None,yaml_files=None, key=None, ex_value=None, default=None, groupnames=None):
        ...
        # 此分支用于使用键进行模糊查询,ex_value用于分支走向的判断
        if ex_value == None:
            lis = []
            try:
                ...
                # 先将列表拆成一个个字典
                for i in data:
                    # 如果val不为None,则走get_dic_nest里的这个if values is not None分支,如果查询到就返回查到的键
                    a = nestselectutil().get_dic_nest(i,objkey=key)
                    if key == a:
                        lis.append(i)
                return lis
            except:
        ...
        try:
            ...
            for key, value in dic1.items():
                if key == objkeys:
                    ...
                    if isinstance(value, dict):
                        new_value = Yamlutil().get_dic_nest(value, objkeys, default)
                        if new_value is not default:
                            return new_value
            return default
        except:
...
if __name__ == '__main__':
    ...
    #         "msg": "操作成功"}
    # b=nestselectutil().get_dic_nest_val(data,objkeys='userInfo')
    # c=nestselectutil().diguichaxun(lis_data=b['tagList'],ex_value=250)
    # print(c)
